...
public class RobotPlayer {
	...
	public static void execute_mission() throws GameActionException {
		...
		case DEFEND:
			if(rcf.getLocation().equals(target)) {
				if(rcf.senseNearbyGameObjects(Robot.class, RobotType.SOLDIER.sensorRadiusSquared, rcf.getTeam().opponent()).length == 0) {
					mission = Order.WAITING;
					centerPoint = find_closest_pasture(rcf.getLocation());
				}
			} else {
				move_to(bug_nav_next(target), false, enemy_hq);
			}
		...
		default:
			...
			if(enemy_pastures.length > 0) {
				...
				ArrayList<MapLocation> enemy_broadcasters = new ArrayList<MapLocation>(20);
				for(MapLocation m : rcf.senseBroadcastingRobotLocations(rcf.getTeam().opponent())) {
					if(!m.equals(enemy_hq)) {
						enemy_broadcasters.add(m);
					}
				}
				if(enemy_broadcasters.size() > 0) {
					move_to(bug_nav_next(enemy_broadcasters.get(0)), sneak, enemy_hq);
				} else {
	...
	public static boolean battlecode() throws GameActionException {
		...
		MapLocation closest_friend = null;
		MapLocation temp_loc;
		int min_dist = 9999;
		int dist = 10000;
		int num_baddies = 0;
		...
		if(num_baddies + num_targets > 0) {
			...
			if(rcf.canAttackSquare(enemy_loc)) {
				if(my_health > 20) {
					...
					if(rcf.getLocation().distanceSquaredTo(enemy_loc) <= 4 &&
							rcf.senseNearbyGameObjects(Robot.class, enemy_loc,(int) (attackRadius + 2*sqrt_attackRadius + 1), rcf.getTeam()).length > rcf.senseNearbyGameObjects(Robot.class,(int) (attackRadius + 2*sqrt_attackRadius + 1), rcf.getTeam().opponent()).length) {
						// avoid suicide bomb
				...
				if(num_friends < num_baddies || my_health < 25) {
					...
					if(istarget || rcf.getLocation().add(enemy_dir).distanceSquaredTo(enemy_loc) > RobotType.SOLDIER.attackRadiusMaxSquared) {
						...
						if(rcf.senseNearbyGameObjects(Robot.class, enemy_loc,(int) (attackRadius + 2*sqrt_attackRadius + 1), rcf.getTeam()).length >= 1) {
							if(rcf.canMove(enemy_dir) && 
									rcf.senseNearbyGameObjects(Robot.class, rcf.getLocation().add(enemy_dir), attackRadius, rcf.getTeam().opponent()).length == 1  ){
								rcf.setIndicatorString(1, "Attacking vulnerable enemy");
								move_to(enemy_dir, false, enemy_hq);
							} else {
								...
								if(rcf.canMove(enemy_dir_left) && 
										rcf.senseNearbyGameObjects(Robot.class, rcf.getLocation().add(enemy_dir_left), attackRadius, rcf.getTeam().opponent()).length == 1  ){
									rcf.setIndicatorString(1, "Attacking vulnerable enemy");
									move_to(enemy_dir_left, false, enemy_hq);
								} else {
									if(rcf.canMove(enemy_dir_right) && 
											rcf.senseNearbyGameObjects(Robot.class, rcf.getLocation().add(enemy_dir_right), attackRadius, rcf.getTeam().opponent()).length == 1  ){
										rcf.setIndicatorString(1, "Attacking vulnerable enemy");
	...
	public static ArrayList<MapLocation> find_sweet_pasture_locations() {
		...
		Comparator<MapLocation> compy = new Comparator<MapLocation>() {
			...
			public int compare(MapLocation arg0, MapLocation arg1) {
				return arg0.distanceSquaredTo(centerPoint) - arg1.distanceSquaredTo(centerPoint);
			}
	...
	public static void hq_signals() throws GameActionException {
		...
		if(rcf.readBroadcast(HQ_VIABILITY_CHANNEL) == 0) {
			...
			if(disadvantage > 0) {
				...
				if(hq_viable && Clock.getRoundNum() > 300 && !hq_constructed ) {
					for(Direction dir : allDirections) {
						if(rcf.canMove(dir) && rcf.canMove(dir.rotateRight())){ 
							rcf.broadcast(PASTR_COUNT_CHANNEL, rcf.readBroadcast(PASTR_COUNT_CHANNEL) + 1);
							broadcast_mission(hq_loc.add(dir), Order.NOISETOWER);
			...
			//				for(int i=0; i<10; i++) {
				//				broadcast_mission(hq_loc.add(Direction.SOUTH), Order.DEFEND);
					//		}
							hq_constructed = true;
							return;
						}
				...
				if(sweet_pasture_locations == null) {
					...
					rcf.setIndicatorString(2, "Pasture Locations: " + sweet_pasture_locations.size());
					if(pasture_index < sweet_pasture_locations.size() && Clock.getRoundNum() > 300) {
						rcf.setIndicatorString(1, "called pasture");

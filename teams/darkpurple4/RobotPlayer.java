...
public class RobotPlayer {
	...
	public static void move_to(Direction preferred_direction, boolean sneak, MapLocation avoiding) throws GameActionException {
		...
		do {
			if(avoiding != null) {
				leftavoidsHQ = (rcf.getLocation().add(final_dir_left).distanceSquaredTo(avoiding) > RobotType.HQ.attackRadiusMaxSquared );
				rightavoidsHQ = (rcf.getLocation().add(final_dir_right).distanceSquaredTo(avoiding) > RobotType.HQ.attackRadiusMaxSquared);
			}
		...
		default:
			...
			if(enemy_pastures.length > 0) {
				...
				if(enemy_broadcasters.length > 0) {
					...
					move_to(bug_nav_next(rcf.senseEnemyHQLocation()), true, enemy_hq);
				}
			}
			return;
	...
	*/
	...
	public static boolean hq_fight() throws GameActionException {
		Robot[] enemies = rcf.senseNearbyGameObjects(Robot.class, RobotType.HQ.attackRadiusMaxSquared, rcf.getTeam().opponent());
		boolean shot = false;
		if(enemies.length > 0) {
			for(Robot enemy : enemies) {
				...
				if(rcf.canAttackSquare(enemy_loc)) {
					rcf.attackSquare(enemy_loc);
					shot = true;
					// should create action delay, so increment counter
					break;
				}
			}
		}
